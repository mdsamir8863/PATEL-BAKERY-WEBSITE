...
class QuantityInput extends HTMLElement {
    constructor() {
        ...
        this.elFakeinput = this.querySelector('input[type="number"]'); // Fake input
        this.elSelect = this.querySelector('select'); // Fake input
        this.elHiddenInput = this.querySelector('input[type="hidden"]'); // real input
        ...
        if (!!this.elFakeinput) this.elFakeinput.addEventListener('input', this.onButtonClick.bind(this))
        if (!!this.elSelect) this.elSelect.addEventListener('input', this.onSelectChange.bind(this));
        this.setQuantity(this.elFakeinput.value)
    }

    onButtonClick(event) {
        event.preventDefault();
        if (event.target.tagName === 'BUTTON') {
            event.target.name === 'plus' ? this.elFakeinput.stepUp() : this.elFakeinput.stepDown();
        }
        this.setQuantity(this.elFakeinput.value)
    }
    ...
    onSelectChange(event) {
        event.preventDefault();
        this.setQuantity(this.elSelect.value)
    }

    setQuantity(value) {
        if (value === '') return
        const previousValue = this.elHiddenInput.value;
        this.elFakeinput.value = 1; // reset value, on reload page text display !== from value stored
        this.elFakeinput.value = value;
        this.elHiddenInput.value = value;
        if (previousValue !== this.elHiddenInput.value) {
            this.changeEvent.detail.quantity = value;
            this.elHiddenInput.dispatchEvent(this.changeEvent);
        }
    }
}
...
class VariantSelects extends HTMLElement {
    constructor() {
        ...
        this.elProductPage = document.querySelector('.js-product');

        this.changeEvent = new CustomEvent('changeVariant', {
            bubbles: true,
            detail: {
    ...
    updateMasterId() {
        ...
        if (this.currentVariant !== currentVariant) {
            this.changeEvent.detail.variant = this.currentVariant
    ...
    toggleAddButton(disable = true, text, modifyClass = true) {
        ...
        if (disable) {
            ...
            if (text) {
                ...
                this.elProductPage.classList.add('is-sold-out');
            }
        } else {
            this.elProductPage.classList.remove('is-sold-out');
